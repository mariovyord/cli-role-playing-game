...
public abstract class Creature implements Combatant {
  ...
  public void attack(Combatant enemy) {
    CliMachine.print(this.name + " attacks " + enemy.getName() + " for " + damage + " damage!");
  ...
  public void printItems() {
    for (int i = 0; i < items.size(); i++) {
      CliMachine.print(i + ": " + items.get(i).getName());
    }
  }
  ...
  public void printItem(int index) {
    if (index < 0 || index >= items.size()) {
      CliMachine.print(INVALID_ITEM_INDEX_MESSAGE);
      return;
    }
    CliMachine.print(items.get(index).getName());
  }
